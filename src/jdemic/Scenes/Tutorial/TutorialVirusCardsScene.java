...
public class TutorialVirusCardsScene {
    ...
    private ImageView createCard(String eventCard) {
        ImageView card = new ImageView(new Image(getClass().getResource("/virusCards/VirusCard" + eventCard + ".png").toExternalForm()));
        ...
        card.setOnMouseClicked(e -> openEventCard(eventCard));
        return card;
    }
    private void openEventCard(String eventCardName) {
        ...
        try {
            ...
            String path = "/virusCards/VirusCard" + cleanName + ".png";
            card = new ImageView(new Image(getClass().getResource(path).toExternalForm()));
        } catch (Exception e) {
            Label fallback = new Label(eventCardName);
    ...
    private void setupUI() {
        ...
        Label panelText = TextUtil.createText("CLICK ON AN\nVIRUS CARD TO SEE\nTHEIR POWERS","hkmodular",0.010,"#00b5d4",root);
        ...
        panelContent.paddingProperty().bind(Bindings.createObjectBinding(() -> new Insets(
        ...
        TutorialUtil.addBottomButtons(root,root, stage,() -> stage.getScene().setRoot(new TutorialEventCardsScene(stage).getRoot()), () -> stage.getScene().setRoot(new TutorialEpidemicScene(stage).getRoot()));
    }
