...
public class PlayScene {
    ...
    // GraphicsContext2D (Canvas) ile frame çizimi
    ...
    // SCREEN 1: LOBBY giriş (HOST GAME / JOIN BY CODE)
    private void showEntryScreen() {
        ...
        // JOIN BY CODE tarafına dokunmuyoruz.
        // joinBtn için bilinçli olarak handler eklenmedi.
        ...
        content.paddingProperty().bind(Bindings.createObjectBinding(
                () -> new Insets(root.getHeight() * 0.10, 0, 0, 0),
                root.heightProperty()
        ));
   ...
   private void showHostCodeScreen() {
    ...
    inner.paddingProperty().bind(Bindings.createObjectBinding(
            () -> new Insets(0, root.getWidth() * 0.010, 0, root.getWidth() * 0.010),
            root.widthProperty()
    ));
